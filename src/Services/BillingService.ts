import { BillingEvent, IPlan, Plan } from "../Models/Subscriptions";
import { IOrganization, Organization } from "../Models/User";
import { BillingStatus, FEATURE_PRICES } from "../Types/payment";
...
export class BillingService {
  ...
  async initializeCardCapture(organizationId: string, features: string[]): Promise<{
    ...
    const result = await paystackService.initializeTransaction(org.email, 5000);
  ...
  async completeCardCapture(
    ...
    org.paystackAuthorizationCode =
      encrypt(verification.authorization.authorization_code);
    await org.save();
  ...
  async checkTrialStatus(organizationId: string): Promise<{
    ...
    if (org.billingStatus !== BillingStatus.TRIAL) {
      return {
        isValid: org.billingStatus === BillingStatus.ACTIVE,
        daysRemaining: 0,
  ...
  async convertTrialToSubscription(
    ...
    const subscription = await paystackService.createSubscription(
      ...
      decrypt(org.paystackAuthorizationCode),
    );
  ...
  async upgradeFeatures(
    ...
    if (prorationAmount > 0) {
      await paystackService.chargeAuthorization(
        ...
        prorationAmount,
        decrypt(org.paystackAuthorizationCode!),
        `proration_${org._id}_${Date.now()}`,
      );
    }

    // Step 2: Get current subscription to disable it
    const currentSubscription = await paystackService.getSubscription(
    ...
    const newSubscription = await paystackService.createSubscription(
      org.paystackCustomerCode!,
  ...
  async applyPendingDowngrade(organizationId: string): Promise<void> {
    const org = await Organization.findById(organizationId);
    if (!org || !org.pendingPlanChange) return;
    ...
    // Get the plan
    const plan = await Plan.findOne({ planCode: pendingChange.planCode });
    if (!plan) throw new Error("Pending plan not found");
  ...
  async handlePaymentFailure(organizationId: string): Promise<void> {
    const org = await Organization.findById(organizationId);
    if (!org) return;
    ...
    org.billingStatus = BillingStatus.PAST_DUE;
  ...
  async handleGraceExpiration(organizationId: string): Promise<void> {
    const org = await Organization.findById(organizationId);
    if (!org) return;
  ...
  getDaysRemainingInCycle(org: IOrganization): number {
  ...
  async findOrCreatePlan(amount: number, features: string[]): Promise<IPlan> {
    // Check if plan with exact amount and features exists
    let plan = await Plan.findOne({ amount, features, isActive: true });
