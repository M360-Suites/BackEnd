 ...
 */
export const getPage = asyncHandler(async (req: CustomRequest, res: Response) => {
  try {
    const { id } = req.params;
    ...
    return resSender(res, 200, 'success', 'Page fetched successfully', null, page);
  } catch (error: any) {
    console.error('Error fetching page:', error);
    return resSender(res, 500, 'error', error.message || 'Failed to fetch page');
  }
});
