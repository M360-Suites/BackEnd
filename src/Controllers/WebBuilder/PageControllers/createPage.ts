 ...
 * Create a new page
 */
export const createPage = asyncHandler(async (req: CustomRequest, res: Response) => {
  try {
    const { title, content, status, seo, websiteId, featuredImageId, mediaIds } = req.body;
    const userId = (req.user as any)._id;
    const orgId = req.organizationId?._id;

    if (!isValidObjectId(websiteId)) return resSender(res, 400, 'error', 'Invalid website ID');
    ...
    if (mediaIds && mediaIds.length > 0) {
      ...
      if (mediaItems.length !== mediaIds.length) {
        return resSender(res, 404, 'error', 'One or more media items not found');
      }
    }
    ...
    if (existingPage) {
      return resSender(res, 409, 'fail', 'Page with this slug already exists');
    }
    // Save the new page
    ...
    return resSender(res, 201, 'success', 'Page created successfully', null, savedPage);
  } catch (error: any) {
    console.error(`Error creating page: ${error}`);
    return resSender(res, 500, 'error', error.message || 'Failed to create page');
  }
});
