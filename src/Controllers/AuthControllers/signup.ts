...
export const signup = asyncHandler(
  async (req: CustomRequest, res: Response) => {
    try {
      ...
      if (existingMail)
        return resSender(res, 403, "fail", "Email Address is already in use");

      // Check token validity
      const decoded: any = await verifyToken(token, jwtAccess);
      // console.log("Decoded payload: ", decoded);
      if (!decoded)
        return resSender(res, 403, "fail", "Validation Token is invalid");
      ...
      if (org) {
        ...
        const newOrg = await billingService.createOrganizationWithTrial(name, email, newUser._id as string);
