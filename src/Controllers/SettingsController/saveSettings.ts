...
export const saveAll = asyncHandler(
...
export const saveGeneral = asyncHandler(
  async (req: CustomRequest, res: Response) => {
    try {
...
export const saveNotification = asyncHandler(
  async (req: CustomRequest, res: Response) => {
    try {
      const userId = (req.user as any)._id;
      ...
      const { error } = Joi.object({
        ...
        sms: Joi.bool(),
        push: Joi.bool(),
        notificationType: Joi.array().items(
...
export const saveDomain = asyncHandler(
  async (req: CustomRequest, res: Response) => {
    try {
      const userId = (req.user as any)._id;
...
export const saveEmail = asyncHandler(
  async (req: CustomRequest, res: Response) => {
    try {
      const userId = (req.user as any)._id;
      const { email, allowAdsNotoification, allowSocialNotifications } =
        req.body;
      const { error } = Joi.object({
...
export const saveAds = asyncHandler(
  async (req: CustomRequest, res: Response) => {
    try {
      const userId = (req.user as any)._id;
...
export const saveCommunity = asyncHandler(
  async (req: CustomRequest, res: Response) => {
    try {
      const userId = (req.user as any)._id;
...
export const saveSecurity = asyncHandler(
  async (req: CustomRequest, res: Response) => {
    try {
      const userId = (req.user as any)._id;
...
export const removeAdsAccount = asyncHandler(
  async (req: CustomRequest, res: Response) => {
    try {
      const userId = (req.user as any)._id;
      ...
      if (!connection)
        return resSender(res, 403, "fail", "Connected account not found!");
      ...
      return resSender(
        ...
        "error",
        "Failed",
        error.message || "Failed to remove ads account"
      );
    }
  }
);
...
export const removeComAccount = asyncHandler(
  async (req: CustomRequest, res: Response) => {
    try {
      const userId = (req.user as any)._id;
