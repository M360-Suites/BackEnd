...
export const fetchAllSubscribers = asyncHandler(async (req: CustomRequest, res: Response) => {
  try {
    ...
    return resSender(res, 200, 'success', 'Subscribers fetched successfully', null, response);
  } catch (error: any) {
    console.error('Error fetching subscribers:', error);
    return resSender(res, 500, 'error', error.message || 'Error fetching subscribers');
  }
});
