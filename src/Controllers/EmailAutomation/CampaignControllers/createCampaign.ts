...
export const createCampaign = asyncHandler(
  async (req: CustomRequest, res: Response) => {
    try {
      ...
      const { type, name, subject, recipients, contents, files, links } =
        req.body;
      let userCred;
      ...
      try {
        ...
        const decryptedUserCred = {
          ...
          accessToken: userCred.accessToken
            ? decrypt(userCred.accessToken)
            : undefined,
          refreshToken: userCred.refreshToken
            ? decrypt(userCred.refreshToken)
            : undefined,
          smtpPassword: userCred.smtpPassword
            ? decrypt(userCred.smtpPassword)
            : undefined,
        };
        ...
        for (const mail of recipientEmails) {
          ...
          const validation = emailSender.validateEmailDataOnly(
            userCred,
            emailData
          );
          if (!validation.valid) {
