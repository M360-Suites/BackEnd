...
export const deleteCampaigns = asyncHandler(async (req: CustomRequest, res: Response) => {
  try {
    ...
    if (!ids || ids.length < 1) return resSender(res, 400, 'fail', 'Campaign IDs are required');
    ...
    return resSender(res, 500, 'error', error.message || 'Error deleting campaigns');
  }
});
