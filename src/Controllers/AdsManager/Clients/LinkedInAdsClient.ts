...
export class LinkedInAdsClient extends AdPlatformClient {
  protected platform = AdsPlatform.LINKEDIN;
  protected baseUrl = "https://api.linkedin.com/rest";
  ...
  async getAdAccount(accountId: string): Promise<AdAccount> {
    const response = await this.makeRequest<any>(
      "GET",
      this.getUrl(`adAccounts/${accountId}`),
      null,
      { headers: this.getHeaders() },
    );
  ...
  async getCampaigns(params?: {
    ...
    return (response.elements || []).map((group: any) => ({
  ...
  async getCampaign(campaignId: string): Promise<Campaign> {
    const response = await this.makeRequest<any>(
      "GET",
      this.getUrl(`adCampaignGroups/${campaignId}`),
      null,
      { headers: this.getHeaders() },
    );
  ...
  async getAdSets(
    ...
    return (response.elements || []).map((campaign: any) => ({
      ...
      name: campaign.name,
      campaign_id: campaign.campaignGroup?.replace(
  ...
  async getAdSet(adSetId: string): Promise<AdSet> {
    const response = await this.makeRequest<any>(
      "GET",
      this.getUrl(`adCampaigns/${adSetId}`),
      null,
      { headers: this.getHeaders() },
    );
    ...
    return {
      id: response.id?.toString(),
      name: response.name,
      campaign_id: response.campaignGroup?.replace(
        "urn:li:sponsoredCampaignGroup:",
        "",
      ),
      status: response.status,
  ...
  async getAds(
    ...
    return (response.elements || []).map((creative: any) => ({
  ...
  async getCreatives(params?: { limit?: number }): Promise<AdCreative[]> {
    return this.getAds(undefined, params) as any;
  }
  ...
  async getAudiences(params?: { limit?: number }): Promise<Audience[]> {
    ...
    return (response.elements || []).map((segment: any) => ({
  ...
  async getAudience(audienceId: string): Promise<Audience> {
    const response = await this.makeRequest<any>(
      "GET",
      this.getUrl(`dmpSegments/${audienceId}`),
      null,
      { headers: this.getHeaders() },
    );
  ...
  async getCampaignInsights(
    ...
    return (response.elements || []).map((item: any) =>
      this.parseLinkedInInsights(item, "campaign"),
    );
  }
  ...
  async getAdSetInsights(
    ...
    return (response.elements || []).map((item: any) =>
      this.parseLinkedInInsights(item, "adset"),
    );
  }
  ...
  async getAdInsights(
    ...
    return (response.elements || []).map((item: any) =>
      this.parseLinkedInInsights(item, "ad"),
    );
  }
  ...
  async getInsights(params: InsightsParams): Promise<InsightsResult[]> {
    ...
    return (response.elements || []).map((item: any) =>
      this.parseLinkedInInsights(item, level),
    );
  }

  private parseLinkedInInsights(item: any, level: string): InsightsResult {
    ...
    return {
      [level === "campaign"
        ...
        : level === "adset"
          ? "adset_id"
          : "ad_id"]: item.pivotValue?.replace(
        /urn:li:sponsored(CampaignGroup|Campaign|Creative):/,
        "",
      ),
