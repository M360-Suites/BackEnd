...
export class GoogleAdsClient extends AdPlatformClient {
  protected platform = AdsPlatform.GOOGLE;
  ...
  async getAdAccounts(): Promise<AdAccount[]> {
    const response = await this.makeRequest<any>(
      "GET",
      this.getUrl("customers:listAccessibleCustomers"),
      null,
  ...
  async getAdAccount(accountId: string): Promise<AdAccount> {
    ...
    const response = await this.makeRequest<any>(
      "POST",
      this.getUrl(`customers/${customerId}/googleAds:searchStream`),
      {
        query: `
          SELECT
            customer.id,
  ...
  async getCampaigns(params?: {
    ...
    return (response[0]?.results || []).map((result: any) => ({
  ...
  async createCampaign(params: CampaignCreateParams): Promise<Campaign> {
    ...
    const budgetResponse = await this.makeRequest<any>(
      "POST",
      this.getUrl(`customers/${customerId}/campaignBudgets:mutate`),
      {
        operations: [
          {
            create: {
              ...
              amountMicros: Math.round(
                (params.daily_budget || 1000) * 1000000,
              ).toString(),
    ...
    const response = await this.makeRequest<any>(
      "POST",
      this.getUrl(`customers/${customerId}/campaigns:mutate`),
      {
        operations: [
          {
            create: {
              ...
              advertisingChannelType: this.mapObjectiveToChannelType(
                params.objective,
              ),
  ...
  async updateCampaign(
    ...
    const updateData: any = { resourceName };
  ...
  async createAdSet(params: AdSetCreateParams): Promise<AdSet> {
    ...
    const response = await this.makeRequest<any>(
      "POST",
      this.getUrl(`customers/${customerId}/adGroups:mutate`),
      {
        operations: [
          {
            create: {
              ...
              ...(params.bid_amount && {
                cpcBidMicros: Math.round(
                  params.bid_amount * 1000000,
                ).toString(),
              }),
            },
          },
  ...
  async updateAdSet(
    ...
    if (params.bid_amount) {
      updateData.cpcBidMicros = Math.round(
        params.bid_amount * 1000000,
      ).toString();
  ...
  async getAds(
    ...
    if (adSetId) {
      conditions.push(
        `ad_group_ad.ad_group = 'customers/${customerId}/adGroups/${adSetId}'`,
      );
    }
    if (params?.status) {
  ...
  async createAd(params: AdCreateParams): Promise<Ad> {
    ...
    const response = await this.makeRequest<any>(
      "POST",
      this.getUrl(`customers/${customerId}/adGroupAds:mutate`),
      {
        operations: [
          {
            create: {
  ...
  async createCreative(params: AdCreativeCreateParams): Promise<AdCreative> {
    ...
    const response = await this.makeRequest<any>(
      "POST",
      this.getUrl(`customers/${customerId}/assets:mutate`),
      {
        operations: [
          {
            create: {
  ...
  async createAudience(params: AudienceCreateParams): Promise<Audience> {
    ...
    const response = await this.makeRequest<any>(
      "POST",
      this.getUrl(`customers/${customerId}/userLists:mutate`),
      {
        operations: [
          {
            create: {
  ...
  async getInsights(params: InsightsParams): Promise<InsightsResult[]> {
    ...
    return (response[0]?.results || []).map((result: any) =>
      this.parseGoogleInsights(result, level),
    );
  }

  private parseGoogleInsights(result: any, level: string): InsightsResult {
