 ...
 */
export const listMedia = asyncHandler(async (req: CustomRequest, res: Response) => {
  try {
    ...
    const { error } = Joi.object({
      ...
      mimeType: Joi.string().valid('image', 'video', 'audio', 'document').optional(),
    }).validate(req.query);
    ...
    return resSender(res, 500, 'error', error.message || 'Failed to list media');
  }
});
