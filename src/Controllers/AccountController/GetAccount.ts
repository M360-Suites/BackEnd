...
export const getAuthAccount = asyncHandler(async (req: CustomRequest, res: Response) => {
  try {
    const user = req.user;
    return resSender(res, 204, 'success', 'Fetched!', 'User account fetched', user);
  } catch (error: any) {
...
export const getAccountDetails = asyncHandler(async (req: CustomRequest, res: Response) => {
  try {
...
export const editProfile = asyncHandler(async (req: CustomRequest, res: Response) => {
  try {
    ...
    return resSender(res, 200, 'success', 'Setting saved', null, modifyUserResponse(user));
  } catch (error: any) {
...
export const inviteAction = asyncHandler(async (req: CustomRequest, res: Response) => {
  try {
    ...
    if (error) return resSender(res, 400, 'fail', 'Wrong Parameter', error.details[0].message);
    ...
    return resSender(res, 500, 'error', error.message || 'Failed to update invite');
  }
});
