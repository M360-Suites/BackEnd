...
export async function subscriptionGuard(
  ...
  try {
    ...
    if (!organizationId) {
      resSender(res, 403, "fail", "No organization specified");
      return;
    }
    ...
    if (!org) {
      resSender(res, 404, "fail", "Organization not found!");
      return;
    }
    ...
    if (org.billingStatus === BillingStatus.PAST_DUE) {
    ...
    if (org.billingStatus === BillingStatus.CANCELED) {
      resSender(
        res,
        403,
        "fail",
    ...
    resSender(res, 500, "error", error.message || "Server error");
  }
}
...
export const requireFeature = (feature: string) => {
  return async (req: CustomRequest, res: Response, next: NextFunction) => {
    ...
    if (!org) {
      return resSender(res, 403, "fail", "Organization not found");
    }
    ...
    if (!org.activeFeatures.includes(feature)) {
      return resSender(
        ...
        `This feature requires the "${feature}" add-on. Please upgrade your plan.`,
