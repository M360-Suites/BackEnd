...
exports.sendEmail = exports.sendMail = exports.unifiedMailService = exports.UnifiedMailService = void 0;
...
class UnifiedMailService {
    constructor() {
        ...
        this.normalTransporter = nodemailer_1.default.createTransport({
            host: smtp,
            port: 465,
            secure: true,
            auth: {
        ...
        this.noReplyTransporter = nodemailer_1.default.createTransport({
            host: smtp,
            port: 465,
            secure: true,
            auth: {
     ...
     */
    async sendEmail(token, emailData) {
     ...
     */
    async sendViaGmail(token, emailData) {
        try {
            const redirectUri = `${this.callBackUrl}/api/campaigns/google/callback`;
            const oauth2Client = new googleapis_1.google.auth.OAuth2(this.clientId, this.clientSecret, redirectUri);
     ...
     */
    createGmailRawMessage(from, emailData) {
        ...
        if (emailData.attachments && emailData.attachments.length > 0) {
            ...
            // Email body part
            message += `--${boundary}\r\n`;
            message += `Content-Type: ${emailData.isHtml ? 'text/html' : 'text/plain'}; charset="UTF-8"\r\n`;
     ...
     */
    async sendViaMicrosoft(token, emailData) {
        try {
     ...
     */
    async sendViaZoho(token, emailData) {
        try {
            if (!token.providerId) {
            ...
            const mailEndpoint = token.location === 'eu'
                ? 'https://mail.zoho.eu'
                : token.location === 'in'
                    ? 'https://mail.zoho.in'
                    : 'https://mail.zoho.com';
            ...
            if (emailData.cc)
                payload.ccAddress = emailData.cc;
            if (emailData.bcc)
                payload.bccAddress = emailData.bcc;
            const response = await axios_1.default.post(`${mailEndpoint}/api/accounts/${token.providerId}/messages`, payload, {
                headers: {
            ...
            if (error.response?.status === 404) {
                throw new Error('Zoho Mail account not found. Please ensure the user has a valid Zoho Mail account.');
            }
    ...
    async refreshMicrosoftToken(token) {
        if (!token.refreshToken) {
            throw new Error('Refresh token missing - reauthentication required');
        }
        try {
     ...
     */
    async sendViaSmtp(token, emailData) {
        ...
        return new Promise((resolve, reject) => {
            transporter.sendMail(mailOptions, (error, info) => {
                if (error) {
                    ...
                    logger_1.logger.info('SMTP email sent successfully');
     ...
     */
    async sendViaInternalEmail(transporter, fromEmail, emailData) {
        ...
        return new Promise((resolve, reject) => {
            transporter.sendMail(mailOptions, (error, info) => {
                if (error) {
                    ...
                    logger_1.logger.info('Internal email sent successfully!');
                    console.log('Message ID:', info.messageId);
     ...
     */
    createNodemailerOptions(from, emailData) {
        const options = {
            from,
            to: emailData.to,
            subject: emailData.subject,
     ...
     */
    async sendSimpleEmail(token, to, subject, body) {
     ...
     */
    async sendHtmlEmail(token, to, subject, htmlBody) {
        return this.sendEmail(token, {
            to,
            subject,
            body: htmlBody,
     ...
     */
    async sendMail(mailRecipient, subject, mailContent, mailSender) {
        const token = {
     ...
     */
    validateEmailDataOnly(token, emailData) {
        const errors = [];
        try {
...
exports.UnifiedMailService = UnifiedMailService;
// Create a singleton instance
const unifiedMailService = new UnifiedMailService();
exports.unifiedMailService = unifiedMailService;
// Legacy exports for backward compatibility
exports.sendMail = unifiedMailService.sendMail.bind(unifiedMailService);
exports.sendEmail = unifiedMailService.sendEmail.bind(unifiedMailService);
