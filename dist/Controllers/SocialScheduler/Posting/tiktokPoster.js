...
exports.TikTokPoster = void 0;
const axios_1 = __importDefault(require("axios"));
const basePoster_1 = require("./basePoster");
const fs = __importStar(require("fs"));
class TikTokPoster extends basePoster_1.BasePoster {
    constructor() {
        super(...arguments);
        this.MIN_CHUNK_SIZE = 5 * 1024 * 1024; // 5MB minimum
        this.MAX_CHUNK_SIZE = 64 * 1024 * 1024; // 64MB maximum
        this.FINAL_CHUNK_MAX = 128 * 1024 * 1024; // 128MB max for final chunk
        this.DEFAULT_CHUNK_SIZE = 10 * 1024 * 1024; // 10MB default
    }
    async post(content) {
        try {
            ...
            if (!content.text && !content.files) {
                ...
                return this.createResult(false, undefined, "TikTok posts require at least text or media content");
            }
            ...
            if (content.files && content.files.length > 0) {
                ...
                if (fileType === "video") {
                    ...
                    return await this.handleVideoUpload(content, accessToken, fileSize, creatorInfo);
                }
            ...
            return this.createResult(false, undefined, `TikTok post failed: ${errorDetails}`);
        }
    }
    async getCreatorInfo(accessToken) {
        try {
            const response = await axios_1.default.post("https://open.tiktokapis.com/v2/post/publish/creator_info/query/", {}, {
                headers: {
            ...
            const errorDetails = this.getErrorDetails(error);
            throw new Error(errorDetails || "Could not verify creator posting permissions");
        }
    }
    async handleVideoUpload(content, accessToken, fileSize, creatorInfo) {
        ...
        for await (const chunk of fileStream) {
            ...
            const response = await axios_1.default.put(uploadUrl, chunk, {
                headers: {
            ...
            if (response.status !== (isFinalChunk ? 201 : 206)) {
                throw new Error(`Unexpected status code ${response.status} during upload`);
            }
    ...
    async handlePhotoPost(content, accessToken, creatorInfo) {
        ...
        const response = await axios_1.default.post("https://open.tiktokapis.com/v2/post/publish/content/init/", {
            media_type: "PHOTO",
...
exports.TikTokPoster = TikTokPoster;
